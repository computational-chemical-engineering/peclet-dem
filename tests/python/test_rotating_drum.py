...
def bed_tilt(pos):
    """Angle of the bed centre of mass about the drum axis, from straight down (deg, + toward +x)."""
    return float(np.degrees(np.arctan2(pos[:, 0].mean() - cx, -(pos[:, 1].mean() - cy))))
...
def test_rotating_drum():
    """Confinement (no grain tunnels through the SDF barrel) and the dynamic response: a CCW-spun
    drum in the rolling regime (Fr = omega^2 R / g ~ 0.3) drags the bed up its +x side, so the
    bed tilts toward +x versus the static drum's symmetric bed. (omega = 2 rad/s, Fr = 5, is a
    centrifuging bed whose centre of mass is not a stable observable.)"""
    omega = 0.5
    sim, wid, N = build()
    pen0, tilt0 = settle_and_spin(sim, wid, 0.0, 2500)
    print(f"static  : N={N} max poke {pen0:+.3f}  bed tilt {tilt0:+.1f} deg")
    sim, wid, N = build()
    penP, tiltP = settle_and_spin(sim, wid, omega, 2500)
    print(f"spinning: omega={omega} (Fr={omega**2 * R / 12.0:.2f}) max poke {penP:+.3f}  "
          f"bed tilt {tiltP:+.1f} deg  (delta {tiltP - tilt0:+.1f})")
    assert penP < 1.5 and pen0 < 1.5, "confinement: grains tunnelled through the barrel"
    assert abs(tilt0) < 5.0, "static bed is not symmetric about the bottom"
    assert tiltP - tilt0 > 5.0, "bed not dragged up the rising side"
