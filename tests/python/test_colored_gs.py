...
the scheme:
  ...
  D. COLOURING INVARIANT — the incremental (warm-started) colouring must never import a conflict.
"""
import numpy as np
import pytest
...
def run_conservation(use_gs=True, seed=0):
...
# ---------------------------------------------------------------------------------------- pytest
@pytest.mark.parametrize("e", [0.2, 0.5, 0.8, 0.95])
def test_binary_exactness(e):
    """A lone (count==1) collision realises the prescribed e exactly and conserves momentum."""
    eff, pd = run_binary(e, use_gs=True)
    print(f"   e={e:.2f}:  e_eff={eff:.4f}   |err|={abs(eff - e):.4f}   momentum_drift={pd:.2e}")
    assert abs(eff - e) < 0.03
    assert pd < 1e-4


def test_conservation_colored_gs():
    """Free cylinder cloud: the colored GS conserves linear AND angular momentum (equal-and-opposite
    impulses at one contact point); the count-averaged Jacobi is reported for contrast."""
    pd_gs, ld_gs, spun = run_conservation(use_gs=True)
    pd_j, ld_j, _ = run_conservation(use_gs=False)
    print(f"   colored GS :  |dP|/|P|={pd_gs:.2e}   |dL|/|L|={ld_gs:.2e}   max|w| imparted={spun:.2f}")
