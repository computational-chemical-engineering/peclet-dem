...
def test_binary_restitution():
    """Head-on equal spheres: the Hertz damping formula realises the prescribed e."""
...
def test_slab_stick():
    """Rotation-locked slab on a tan(theta)=0.5 incline: mu=0.9 sticks, mu=0.05 slides."""
    d_hi = slab_slide(0.9)
...
def test_kinetic_slide():
    """Kinetic Coulomb slide: a = gx - mu*gz, distance = a t^2 / 2 (gx=8, gz=10)."""
    d3 = single_slide(0.3)
    d6 = single_slide(0.6)
    t = 160000 * DT
...
def test_slide_to_roll():
    """A sliding sphere on a frictional floor rolls at v = 5/7 v0."""
    rv, rw = roll_ratio()
