...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    for (int i = 0; i < N; ++i) {
      invMass[i] = upos(rng);
      iIx[i] = upos(rng); iIy[i] = upos(rng); iIz[i] = upos(rng);
      ...
      wx[i]=uf(rng); wy[i]=uf(rng); wz[i]=uf(rng);
    }
    ...
    for (int k = 0; k < M; ++k) {
      ...
      if ((rng() & 7) == 0) {
        ...
        do { b = ubody(rng); } while (b == a);
        if (b < a) { int t=a; a=b; b=t; }  // canonical bodyA<bodyB
      }
      ManifoldC m{};
      m.bodyA = a; m.bodyB = b;
    ...
    auto up3 = [&](const char* nm, const std::vector<float>& x, const std::vector<float>& y,
      ...
      for (int i = 0; i < N; ++i) { h(i,0)=x[i]; h(i,1)=y[i]; h(i,2)=z[i]; }
      Kokkos::deep_copy(v, h);
      return v;
    };
    Kokkos::View<float*, CpMem> dInvMass("invMass", N);
    { auto h=Kokkos::create_mirror_view(dInvMass); for(int i=0;i<N;++i)h(i)=invMass[i]; Kokkos::deep_copy(dInvMass,h);}
    auto dInvI = up3("invI", iIx, iIy, iIz);
    Kokkos::View<float* [4], CpMem> dQuat("quat", N);
    { auto h=Kokkos::create_mirror_view(dQuat); for(int i=0;i<N;++i){h(i,0)=qx[i];h(i,1)=qy[i];h(i,2)=qz[i];h(i,3)=qw[i];} Kokkos::deep_copy(dQuat,h);}
    ...
    { auto h=Kokkos::create_mirror_view(dMan); for(int k=0;k<M;++k)h(k)=man[k]; Kokkos::deep_copy(dMan,h);}
    ...
    for (int k = 0; k < M; ++k) {
      ...
      if (m.num_points <= 0) continue;
      int idA=m.bodyA, idB=m.bodyB, realA=idA, realB=idB;
      if (idB>=0){ realB=idB; if(realA>realB) continue; }
      ...
      if(idB>=0){ vB=F3{vx[realB],vy[realB],vz[realB]}; wB=F3{wx[realB],wy[realB],wz[realB]}; }
      ...
      float lenN=std::sqrt(dot3(Nsum,Nsum)); if(lenN<1e-9f) continue;
      F3 diff=sub3(rAavg,rBavg);
      F3 vG=scale3(diff,growthRate);
      float vn=dot3(vA,Nsum)+dot3(wA,TauA)+dot3(vB,F3{-Nsum.x,-Nsum.y,-Nsum.z})+dot3(wB,TauB);
      vn+=dot3(vG,Nsum);
      float align=dot3(Nsum,diff);
      if(align>0){ if(vn<0) continue; } else { if(vn>0) continue; }
      ...
      float wT=wAn+wBn; if(wT<=0) continue;
      float lambda=(-restitution*vn - vn)/wT;
      F3 Jlin=scale3(Nsum,lambda), JangA=scale3(TauA,lambda), JangB=scale3(TauB,lambda);
      rdv[3*realA]+=Jlin.x*invMA; rdv[3*realA+1]+=Jlin.y*invMA; rdv[3*realA+2]+=Jlin.z*invMA;
      { F3 Jl=invRotateVector(qA,JangA); F3 dwl{Jl.x*invIA.x,Jl.y*invIA.y,Jl.z*invIA.z}; F3 dww=rotateVector(qA,dwl);
        rdw[3*realA]+=dww.x; rdw[3*realA+1]+=dww.y; rdw[3*realA+2]+=dww.z; }
      if(idB>=0){
        rdv[3*realB]+=-Jlin.x*invMB; rdv[3*realB+1]+=-Jlin.y*invMB; rdv[3*realB+2]+=-Jlin.z*invMB;
        F3 Jl=invRotateVector(qB,JangB); F3 dwl{Jl.x*invIB.x,Jl.y*invIB.y,Jl.z*invIB.z}; F3 dww=rotateVector(qB,dwl);
        rdw[3*realB]+=dww.x; rdw[3*realB+1]+=dww.y; rdw[3*realB+2]+=dww.z;
      }
    }
    ...
    for (int i = 0; i < 3*N; ++i) {
      ...
      if (std::fabs(gdv[i]-rdv[i]) > t) ++bad;
      float tw = 1e-4f * (1.0f + std::fabs(rdw[i]));
      if (std::fabs(gdw[i]-rdw[i]) > tw) ++bad;
    }
    if (bad) {
