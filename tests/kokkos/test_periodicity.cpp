...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    std::vector<float> qx(capacity), qy(capacity), qz(capacity), qw(capacity), sc(capacity),
        im(capacity);
    ...
    auto pos = mk3("pos", px, py, pz), posPred = mk3("pp", px, py, pz),
         vel = mk3("vel", vx, vy, vz), velPred = mk3("vp", vx, vy, vz);
    ...
    {
      auto h = Kokkos::create_mirror_view(scale);
      for (int i = 0; i < capacity; ++i)
        h(i) = sc[i];
      Kokkos::deep_copy(scale, h);
    }
    Vi shapeId("sid", capacity), realIdx("ri", capacity);
    {
      auto h = Kokkos::create_mirror_view(shapeId);
      for (int i = 0; i < capacity; ++i)
        h(i) = sid[i];
      Kokkos::deep_copy(shapeId, h);
    }
    Kokkos::View<int, CpMem> top("top");
    Kokkos::deep_copy(top, numReal);
    ...
    auto hpos = h3(pos), hpp = h3(posPred), hvel = h3(vel);
    auto hq = h4(quat);
    auto hri = Kokkos::create_mirror_view(realIdx);
    Kokkos::deep_copy(hri, realIdx);
    auto hsc = Kokkos::create_mirror_view(scale);
    Kokkos::deep_copy(hsc, scale);
    auto hsid = Kokkos::create_mirror_view(shapeId);
    Kokkos::deep_copy(hsid, shapeId);
    ...
    for (int g = numReal; g < total; ++g) {
      ...
      float kx = (hpos(g, 0) - px[r]) / dom.size.x, ky = (hpos(g, 1) - py[r]) / dom.size.y,
            kz = (hpos(g, 2) - pz[r]) / dom.size.z;
      auto okk = [&](float k) {
        float rk = std::round(k);
        return std::fabs(k - rk) < 1e-3f && std::fabs(rk) <= 1.0001f;
      };
      if (!okk(kx) || !okk(ky) || !okk(kz)) {
        ++bad;
        continue;
      }
      if (std::round(kx) == 0 && std::round(ky) == 0 && std::round(kz) == 0) {
        ++bad;
        continue;
      }  // not a self-copy
      // posPred shifted by same offset
      if (!close(hpp(g, 0), px[r] + std::round(kx) * dom.size.x) ||
          !close(hpp(g, 1), py[r] + std::round(ky) * dom.size.y) ||
          !close(hpp(g, 2), pz[r] + std::round(kz) * dom.size.z)) {
        ++bad;
        continue;
      }
      // verbatim copies
      if (!close(hvel(g, 0), vx[r]) || !close(hvel(g, 1), vy[r]) || !close(hvel(g, 2), vz[r]))
        ++bad;
      if (!close(hq(g, 0), qx[r]) || !close(hq(g, 3), qw[r]))
        ++bad;
      if (!close(hsc(g), sc[r]) || hsid(g) != sid[r])
        ++bad;
    }
    if (bad) {
      std::fprintf(stderr, "FAIL: %d ghosts invalid\n", bad);
      status = 1;
    }
    if (!status)
      std::printf("[periodicity] PASS: %d ghosts from %d reals correct (exec: %s)\n",
