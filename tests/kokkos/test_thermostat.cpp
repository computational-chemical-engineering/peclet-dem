...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    std::vector<float> vx(N), vy(N), vz(N), wx(N), wy(N), wz(N), im(N), iix(N), iiy(N), iiz(N),
        qx(N), qy(N), qz(N), qw(N);
    for (int i = 0; i < N; ++i) {
      vx[i] = uf(rng);
      vy[i] = uf(rng);
    ...
    V4 quat("q", N);
    {
      auto h = Kokkos::create_mirror_view(quat);
      for (int i = 0; i < N; ++i) {
        h(i, 0) = qx[i];
        h(i, 1) = qy[i];
        h(i, 2) = qz[i];
    ...
    {
      auto h = Kokkos::create_mirror_view(vel);
      Kokkos::deep_copy(h, vel);
      for (int i = 0; i < N; ++i) {
        gv[3 * i] = h(i, 0);
        gv[3 * i + 1] = h(i, 1);
        gv[3 * i + 2] = h(i, 2);
    ...
    {
      auto h = Kokkos::create_mirror_view(ang);
      Kokkos::deep_copy(h, ang);
      for (int i = 0; i < N; ++i) {
        gw[3 * i] = h(i, 0);
        gw[3 * i + 1] = h(i, 1);
        gw[3 * i + 2] = h(i, 2);
    ...
    double ndof = 3.0 * N;
    auto lam = [&](double ke) {
      double Tc = (2.0 * ke) / (ndof * kB);
      if (Tc < 1e-6)
        return 1.0;
      double f = 1.0 + (dt / tau) * (Ttarget / Tc - 1.0);
      if (f < 0)
        f = 0;
      return std::sqrt(f);
    };
    ...
    for (int i = 0; i < N; ++i) {
      if (!tol(gv[3 * i], vx[i] * lt) || !tol(gv[3 * i + 1], vy[i] * lt) ||
          !tol(gv[3 * i + 2], vz[i] * lt))
        ++bad;
      if (!tol(gw[3 * i], wx[i] * lr) || !tol(gw[3 * i + 1], wy[i] * lr) ||
          !tol(gw[3 * i + 2], wz[i] * lr))
        ++bad;
    }
    if (bad) {
      std::fprintf(stderr, "FAIL: %d/%d scaled velocities differ (lt=%f lr=%f)\n", bad, 6 * N, lt,
                   lr);
      status = 1;
    } else
      std::printf("[thermostat] PASS: %d bodies rescaled (lt=%.4f lr=%.4f) match host (exec: %s)\n",
                  N, lt, lr, CpExec::name());
  }
  Kokkos::finalize();
  return status;
