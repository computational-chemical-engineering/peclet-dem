...
// borderline contacts), with random orientations, plus a ground plane. We run the device kernels and
// a host loop over the same dem:: math, then require the two contact sets to match (sorted, within
// tolerance). Runs on whatever backend Kokkos was built for.
...
static bool close(float a, float b, float tol) { return std::fabs(a - b) <= tol * (1.0f + std::fabs(b)); }
...
static bool eq_cc(const CC& x, const CC& y) {
  if (x.a != y.a || x.b != y.b || !close(x.dist, y.dist, 1e-4f)) return false;
  for (int d = 0; d < 3; ++d)
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      qx[i] = a / nrm; qy[i] = b / nrm; qz[i] = c / nrm; qw[i] = d / nrm;
      sc[i] = 1.0f;
    ...
    Kokkos::View<int*, CpMem> shapeId("sid", N);
    {
      ...
      for (int i = 0; i < N; ++i) {
        hp(i, 0) = px[i]; hp(i, 1) = py[i]; hp(i, 2) = pz[i];
        hq(i, 0) = qx[i]; hq(i, 1) = qy[i]; hq(i, 2) = qz[i]; hq(i, 3) = qw[i];
        hs(i) = sc[i]; hi(i) = sid[i];
      }
      Kokkos::deep_copy(pos, hp); Kokkos::deep_copy(quat, hq);
      Kokkos::deep_copy(scale, hs); Kokkos::deep_copy(shapeId, hi);
    }
    Kokkos::View<ShapeDesc*, CpMem> dShapes("shapes", shapes.size());
    { auto h = Kokkos::create_mirror_view(dShapes); for (size_t i=0;i<shapes.size();++i) h(i)=shapes[i]; Kokkos::deep_copy(dShapes, h); }
    Kokkos::View<PlaneP*, CpMem> dPlanes("planes", planes.size());
    { auto h = Kokkos::create_mirror_view(dPlanes); for (size_t i=0;i<planes.size();++i) h(i)=planes[i]; Kokkos::deep_copy(dPlanes, h); }
    Kokkos::View<float* [3], CpMem> shell("shell", 1);  // unused (spheres)
    Kokkos::View<int* [2], CpMem> pairs("pairs", numPairs);
    { auto h = Kokkos::create_mirror_view(pairs); for (int i=0;i<numPairs;++i){h(i,0)=pa[i];h(i,1)=pb[i];} Kokkos::deep_copy(pairs, h); }
    ...
    std::vector<CC> got;
    {
      ...
      for (int i = 0; i < nc && i < maxContacts; ++i) got.push_back(toCC(h(i)));
    }
    ...
    for (int idx = 0; idx < numPairs; ++idx) {
      ...
      for (int k = 0; k < iter; ++k) {
        ...
        float eps = 1e-4f;
        F3 nLoc{sdfEval(F3{pCanB.x+eps,pCanB.y,pCanB.z},dB.type,dB.params)-sdfEval(F3{pCanB.x-eps,pCanB.y,pCanB.z},dB.type,dB.params),
                sdfEval(F3{pCanB.x,pCanB.y+eps,pCanB.z},dB.type,dB.params)-sdfEval(F3{pCanB.x,pCanB.y-eps,pCanB.z},dB.type,dB.params),
                sdfEval(F3{pCanB.x,pCanB.y,pCanB.z+eps},dB.type,dB.params)-sdfEval(F3{pCanB.x,pCanB.y,pCanB.z-eps},dB.type,dB.params)};
        float len = len3(nLoc);
        ...
        ref.push_back(CC{idA, idB, {nWorld.x,nWorld.y,nWorld.z}, {rA.x,rA.y,rA.z}, {rB.x,rB.y,rB.z}, effDist});
      }
    }
    for (int i = 0; i < N; ++i) {
      ...
      float baseR = d.params.x; if (baseR == 0.0f) baseR = 1.0f;
      ...
      for (const auto& pl : planes) {
        ...
        ref.push_back(CC{i, -1, {pl.normal.x,pl.normal.y,pl.normal.z}, {rA.x,rA.y,rA.z}, {pl.point.x,pl.point.y,pl.point.z}, dist});
      }
    }
