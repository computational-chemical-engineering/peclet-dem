...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      py[i] = uf(rng) * 5;
      pz[i] = uf(rng) * 5;
      randq(qpx, qpy, qpz, qpw, i);
      randq(qsx, qsy, qsz, qsw, i);
    }
    ...
    auto computeC = [&](const ContactC& c, F3& rA_out, F3& rB_out, F3& n_out) {
      ...
      if (idB >= 0) {
        pB = pp(idB);
        F4 qB = qp(idB);
        F4 qBd = quatMult(qB, quatInverse(qs(idB)));
        rB = rotateVector(qBd, rB);
        n = rotateVector(qBd, n);
      }
      float C;
      if (idB < 0) {
        n = F3{c.normal.x, c.normal.y, c.normal.z};
        C = dot3(sub3(add3(pA, rA), F3{c.rB.x, c.rB.y, c.rB.z}), n);
      } else {
        C = dot3(sub3(add3(pA, rA), add3(pB, rB)), n);
      }
      rA_out = rA;
      rB_out = rB;
      n_out = n;
    ...
    for (int k = 0; k < 4000; ++k) {
      ...
      c.weight = 0;
      F3 ra, rb, n;
      float C = computeC(c, ra, rb, n);
      if (std::fabs(C) < 0.05f)
        continue;  // drop borderline
    ...
    auto up4 = [&](const char* nm, std::vector<float>& x, std::vector<float>& y,
                   std::vector<float>& z, std::vector<float>& w) {
      Kokkos::View<float* [4], CpMem> v(nm, N);
      auto h = Kokkos::create_mirror_view(v);
      for (int i = 0; i < N; ++i) {
        h(i, 0) = x[i];
        h(i, 1) = y[i];
    ...
    std::vector<float> gdp(3 * N), gdq(4 * N);
    std::vector<int> gcn(N);
    float gmo = 0;
    {
      auto h = Kokkos::create_mirror_view(dDpos);
      Kokkos::deep_copy(h, dDpos);
      for (int i = 0; i < N; ++i) {
        gdp[3 * i] = h(i, 0);
        gdp[3 * i + 1] = h(i, 1);
        gdp[3 * i + 2] = h(i, 2);
    ...
    {
      auto h = Kokkos::create_mirror_view(dDquat);
      Kokkos::deep_copy(h, dDquat);
      for (int i = 0; i < N; ++i) {
        gdq[4 * i] = h(i, 0);
        gdq[4 * i + 1] = h(i, 1);
        gdq[4 * i + 2] = h(i, 2);
    ...
    {
      auto h = Kokkos::create_mirror_view(dCount);
      Kokkos::deep_copy(h, dCount);
      for (int i = 0; i < N; ++i)
        gcn[i] = h(i);
    }
    {
      auto h = Kokkos::create_mirror_view(dMaxOv);
      Kokkos::deep_copy(h, dMaxOv);
      gmo = h();
    }

    // --- host reference ---
    std::vector<float> rdp(3 * N, 0), rdq(4 * N, 0);
    std::vector<int> rcn(N, 0);
    float rmo = 0;
    ...
    int bad = 0, badc = 0;
    for (int i = 0; i < 3 * N; ++i)
      if (std::fabs(gdp[i] - rdp[i]) > 1e-4f * (1 + std::fabs(rdp[i])))
        ++bad;
    for (int i = 0; i < 4 * N; ++i)
      if (std::fabs(gdq[i] - rdq[i]) > 1e-4f * (1 + std::fabs(rdq[i])))
        ++bad;
    for (int i = 0; i < N; ++i)
      if (gcn[i] != rcn[i])
        ++badc;
    ...
    if (bad || badc || !moOk) {
      ...
      std::printf(
          "[solver_position] PASS: %d contacts, deltas/counts/max_overlap match host (exec: %s)\n",
          M, CpExec::name());
    }
  }
  Kokkos::finalize();
