// packing-gpu — the distributed (MPI) Kokkos demStep capstone test (the last suite-migration
// piece).
//
...
//                     MPI_COMM_WORLD + stepMpi (gather ghosts -> broad/narrow/solve with
//                     per-iteration owner->ghost forwards). Cross-block body-body contacts are
//                     reconstructed by the transport-core particle halo (ParticleHaloTopology<3> +
//                     ParticleHalo<3>).
//   * single-rank  : the SAME stepMpi code on a size-1 communicator (MPI_COMM_SELF) over ALL
//   particles
...
//              block owns everything (no ghosts needed); the distributed run rebuilds the
//              split-plane neighbours via ghosts.
//   periodic : a FULLY periodic tiled box, so neighbours wrap across every face. This exercises the
//              LOCAL periodic self-ghosts (ParticleHaloTopology build(...,
//              includePeriodicSelf=true)) on every UNDECOMPOSED periodic axis -- all three at np=1,
//              and the z of the np=4 2x2x1 ORB layout (each rank is its own periodic image on a
//              "x1" axis, so those wrap contacts exist only as self-ghosts). A periodic box needs a
//              thick ghost boundary layer, so capacity is sized generously (gather throws on
//              overflow rather than corrupting the SoA).
//
// Validation is WEAKER than the cfd MPI test: the XPBD solver accumulates per-particle deltas via
// atomics, so the decomposition reorders the sums and the result is NOT bit-exact even at np=1.
// closed (spacing < diameter, over-constrained) has an essentially unique packing -> tight position
// parity; periodic (spacing > diameter) has cage slack -> a manifold of zero-overlap configs, so
// the physical check is that the overlap relaxes identically and positions stay bounded (no
// divergence). Per-process: the reference is computed once on rank 0 and broadcast. Build with
// -DDEM_MPI.
#include <mpi.h>
...
// globalScale = radius (scale=1), so the body-body contact distance is ~2*globalScale; RCUT
// generously covers it so every cross-block / periodic-wrap contact is ghosted symmetrically on
// both owners.
...
// Deterministic per-index jitter (a cheap hash -> [-amp,amp]) breaks the lattice symmetry so the
// contact pattern is uneven: if the distributed decomposition reordered the atomic delta
// accumulation away from the single-block reference, the positions would diverge.
...
// A periodic axis TILES the box (L = G*spacing) so neighbours wrap across that face; a non-periodic
// axis centres the lattice with a margin. spacing > diameter keeps the base lattice overlap-free
// (resolvable); jitter then makes some neighbours (incl. across the periodic faces) overlap, which
// the solver relaxes.
...
int main(int argc, char** argv) {
  ...
  int fail = 0, size = 1, rank = 0;
  {
    ...
    // periodic = FULLY periodic tiled box: exercises self-ghosts on every undecomposed periodic
    // axis (all three at np=1; z of the np=4 2x2x1 layout; etc.). A periodic box needs a thick
    // ghost boundary layer, so the KokkosSim capacity below is sized generously (gather throws on
    // overflow).
    const Scene scene = periodic ? Scene{{true, true, true}, 5, 11.0, 2.2}
                                 : Scene{{false, false, false}, 4, 14.0, 1.8};
    const int cap =
        8 * scene.G * scene.G * scene.G + 64;  // ghost headroom (fully-periodic = thick layer)
    ...
    // Verlet-skin ghost reuse (D2): build the halo topology with a band of RCUT+GS and reuse it
    // across substeps until a particle moves > GS. The result must still match the
    // rebuild-every-substep reference (which keeps skin=0 below) — reuse only changes WHEN the
    // topology is rebuilt.
    ...
    if (rank == 0) {
      ...
      ref.initMpi(origin, dsize, gsize, per,
                  MPI_COMM_SELF);  // size-1 block; periodic via self-ghosts
    ...
    // distributed and reference solves, differing only in atomic-accumulation order, can drift
    // along it. Remove that gauge (subtract the mean displacement per axis) before measuring
    // divergence -- the residual is the physically meaningful, gauge-invariant error.
    double lsum[3] = {0, 0, 0};
    auto disp = [&](int i, int c) {
      double d = static_cast<double>(distPos[3 * i + c]) -
                 static_cast<double>(refPos[3 * ownedGid[i] + c]);
      if (scene.per[c])
        d -= L * std::round(d / L);  // minimum image (a particle may wrap a face)
      return d;
    };
    ...
    //  * closed (spacing < diameter, over-constrained): the relaxed packing is essentially unique,
    //  so
    ...
    //    zero-overlap configs, so dist and ref settle into DIFFERENT ones within the slack --
    //    position parity is not meaningful. The physical check is that the overlap relaxes
    //    identically (proves the z-wrap contacts are handled by the self-ghosts; a missing-contact
    //    bug leaves residual overlap ~0.2, and the np=1-fully-periodic instability blows it to ~2).
    //    posErr only guards against divergence (the broken case gives ~5, far beyond the cage
    //    slack).
    const double posTol = periodic ? 0.30 : (size == 1 ? 1e-3 : 1e-2);
    const bool relaxed = !periodic || (distOv < 1e-3 && refOv < 1e-3);
    if (rank == 0)
      std::printf(
          "  [%-8s] np=%d  particles=%d  ghosts(total)=%d  posErr=%.3e (tol %.0e)  overlap "
          "dist=%.4e ref=%.4e d=%.2e\n",
          periodic ? "periodic" : "closed", size, n, totGhost, posErr, posTol, distOv, refOv,
          ovErr);
    if (!(posErr < posTol) || !(ovErr < 5e-3) || !relaxed)
      fail = 1;
    // D2: with a Verlet skin the topology must be reused on at least one substep (fewer rebuilds
    // than gathers) — otherwise the reuse path is never exercised. STEPS=8 gathers, displacement
    // per substep ≪ GS in a relaxing pack, so most substeps reuse.
    if (rank == 0)
