// dem — distributed load re-balancing test (KokkosSim::rebalance / enable_mpi_step rebalance_every).
//
// A skewed particle cloud (density concentrated toward x=0) is distributed over the ORB blocks, so the
// equal-cell decomposition leaves the low-x ranks overloaded. Each particle is given a DISTINCT committed
// state derived deterministically from its position (velocity, angular velocity, orientation, scale,
// inverse inertia). A rebalance re-decomposes by particle count and migrates ownership. We check:
//   (1) every migrated particle's full committed state is preserved bit-for-bit — after the migration a
//       particle at position p still carries exactly f(p) in every field (the SoA pack/unpack round-trips
...
static std::vector<float> skewedPositions() {
  ...
  for (int g = 0; g < N; ++g) {
    ...
    p[3 * g] = (float)x; p[3 * g + 1] = (float)y; p[3 * g + 2] = (float)z;
  }
  return p;
}

// Distinct committed state as deterministic float functions of position (so a particle is identifiable
// by its position after migration, and every field is independently checkable).
static std::array<float, 3> fVel(float x, float y, float z) { return {x * 0.013f, y * 0.017f, z * 0.019f}; }
static std::array<float, 3> fAng(float x, float y, float z) { return {z * 0.011f, x * 0.007f, y * 0.005f}; }
static std::array<float, 3> fInvI(float x, float y, float z) { return {1.0f + 0.001f * x, 1.0f + 0.001f * y, 1.0f + 0.001f * z}; }
static float fScale(float x, float y, float z) { return 1.0f + 0.0001f * (x + y + z); }
...
static void setStateFromPos(KokkosSim& sim, const std::vector<float>& pos) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    auto v = fVel(x, y, z); auto a = fAng(x, y, z); auto ii = fInvI(x, y, z); auto q = fQuat(x, y, z);
    for (int c = 0; c < 3; ++c) { vel[3 * i + c] = v[c]; ang[3 * i + c] = a[c]; invI[3 * i + c] = ii[c]; }
    for (int c = 0; c < 4; ++c) quat[4 * i + c] = q[c];
...
static int verifyState(KokkosSim& sim) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    auto v = fVel(x, y, z); auto a = fAng(x, y, z); auto ii = fInvI(x, y, z); auto q = fQuat(x, y, z);
    for (int c = 0; c < 3; ++c) {
      if (vel[3 * i + c] != v[c]) ++bad;
      if (ang[3 * i + c] != a[c]) ++bad;
      if (invI[3 * i + c] != ii[c]) ++bad;
    }
    for (int c = 0; c < 4; ++c)
      if (quat[4 * i + c] != q[c]) ++bad;
    if (scale[i] != fScale(x, y, z)) ++bad;
  }
  return bad;
}
...
int main(int argc, char** argv) {
  ...
  int fail = 0, size = 1, rank = 0;
  {
    ...
    if (verifyState(sim) != 0) { fail = 1; if (rank == 0) std::fprintf(stderr, "  state corrupted by migration\n"); }
    long lN = newN, gN = 0;
    MPI_Allreduce(&lN, &gN, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
    if (gN != N) { fail = 1; if (rank == 0) std::fprintf(stderr, "  count not conserved: %ld != %d\n", gN, N); }
    const double imb1 = imbalance(newN, MPI_COMM_WORLD);
    if (size > 1 && !(imb0 > 1.2 && imb1 < imb0 && imb1 < 1.6)) {
      fail = 1;
      if (rank == 0) std::fprintf(stderr, "  imbalance not improved: %.3f -> %.3f\n", imb0, imb1);
    }

    // (4a): the step runs and stays finite after an explicit rebalance.
    sim.stepMpi(4);
    float ov = sim.maxOverlap();
    if (!std::isfinite(ov)) { fail = 1; if (rank == 0) std::fprintf(stderr, "  non-finite overlap after rebalance\n"); }

    if (rank == 0)
      std::printf("  np=%d: imbalance %.3f -> %.3f, count %ld, overlap %.3e\n", size, imb0, imb1, gN, ov);
  }
  ...
  // (4b): the wired auto-rebalance path (rebalance_every>0) runs cleanly from a fresh sim.
  {
    ...
    tpx::halo::DomainMap<3> map;
    for (int i = 0; i < 3; ++i) { map.origin[i] = 0; map.cellSize[i] = L / GX; map.periodic[i] = false; }
    tpx::halo::ParticleMigrator<3> mig; mig.init(dec, rank, map, MPI_COMM_WORLD);
    std::vector<float> ownedPos;
    for (int g = 0; g < N; ++g) {
      tpx::Vec<3> x{gpos[3 * g], gpos[3 * g + 1], gpos[3 * g + 2]};
      if (mig.ownerOf(x) == rank) { ownedPos.push_back(gpos[3 * g]); ownedPos.push_back(gpos[3 * g + 1]); ownedPos.push_back(gpos[3 * g + 2]); }
    }
    KokkosSim sim(4 * N);
    configure(sim);
    sim.setPositions(ownedPos);
    ...
    sim.stepMpi(6);
    if (!std::isfinite(sim.maxOverlap())) { fail = 1; if (rank == 0) std::fprintf(stderr, "  auto-rebalance path non-finite\n"); }
    long lc = sim.numParticles(), gc = 0;
    MPI_Allreduce(&lc, &gc, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
    if (gc != N) { fail = 1; if (rank == 0) std::fprintf(stderr, "  auto-rebalance lost particles: %ld\n", gc); }
  }

  int totalFail = 0;
  MPI_Allreduce(&fail, &totalFail, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  if (rank == 0) {
    if (totalFail == 0) std::printf("OK (np=%d): rebalance migrates state intact + improves balance\n", size);
