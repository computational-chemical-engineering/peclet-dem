...
__global__ void splat_particles_kernel(ParticleSystemData ps, float *d_grid,
  ...
  // Min/Max indices
  int min_x =
      max(0, (int)floorf((p_world.x - radius_bound - origin.x) / voxel_size.x));
  int max_x = min(dims.x - 1, (int)ceilf((p_world.x + radius_bound - origin.x) /
                                         voxel_size.x));
  int min_y =
      max(0, (int)floorf((p_world.y - radius_bound - origin.y) / voxel_size.y));
  int max_y = min(dims.y - 1, (int)ceilf((p_world.y + radius_bound - origin.y) /
                                         voxel_size.y));
  int min_z =
      max(0, (int)floorf((p_world.z - radius_bound - origin.z) / voxel_size.z));
  int max_z = min(dims.z - 1, (int)ceilf((p_world.z + radius_bound - origin.z) /
                                         voxel_size.z));
  ...
  for (int z = min_z; z <= max_z; ++z) {
    for (int y = min_y; y <= max_y; ++y) {
      for (int x = min_x; x <= max_x; ++x) {
        ...
        int voxel_idx = z * (dims.x * dims.y) + y * dims.x + x;
...
__global__ void solve_eikonal_jacobi_kernel(float *d_in, float *d_out,
  ...
  // Sample Neighbors
  // Use large value for boundary
  float val_x_m = (x > 0) ? d_in[idx - 1] : 100.0f;
  float val_x_p = (x < dims.x - 1) ? d_in[idx + 1] : 100.0f;
  float val_y_m = (y > 0) ? d_in[idx - stride_y] : 100.0f;
  float val_y_p = (y < dims.y - 1) ? d_in[idx + stride_y] : 100.0f;
  float val_z_m = (z > 0) ? d_in[idx - stride_z] : 100.0f;
  float val_z_p = (z < dims.z - 1) ? d_in[idx + stride_z] : 100.0f;
...
void launch_splat_particles(ParticleSystemData ps, float *d_grid, int *d_state,
  ...
  int block = 128;
  int grid = (ps.num_particles + block - 1) / block;
