...
inline void accumulateNormalImpulseKokkos(Kokkos::View<ContactC*, CpMem> contacts, int numContacts,
  ...
  Kokkos::parallel_for(
      "dem::fric_accum", Kokkos::RangePolicy<CpExec>(space, 0, numContacts), KOKKOS_LAMBDA(int idx) {
        ...
        if (w_n > 1e-6f) contacts(idx).friction_lambda_n += approach / w_n;
      });
  space.fence();
}

/// Plane (idB<0) one-shot loads. Zeros planeFriction first (both columns), then sets the per-contact
/// friction_lambda_n and the per-body max plane load (.x).
...
inline void countFrictionContactsKokkos(Kokkos::View<const ContactC*, CpMem> contacts,
  ...
  Kokkos::parallel_for(
      "dem::fric_count", Kokkos::RangePolicy<CpExec>(space, 0, numContacts), KOKKOS_LAMBDA(int idx) {
        ...
        if (c.bodyB >= 0) Kokkos::atomic_add(&planeFriction(realIdx(c.bodyB), 1), 1.0f);
      });
  space.fence();
}
...
inline void solveContactFrictionKokkos(Kokkos::View<const ContactC*, CpMem> contacts, int numContacts,
                                       Kokkos::View<const float*, CpMem> invMass,
                                       Kokkos::View<const float* [3], CpMem> invInertia,
                                       Kokkos::View<const float* [3], CpMem> velPred,
                                       Kokkos::View<const float* [3], CpMem> angVelPred,
                                       Kokkos::View<const int*, CpMem> realIdx,
                                       FrManifoldCounts planeFriction, float frictionDynamic,
  ...
  Kokkos::parallel_for(
      "dem::fric_solve", Kokkos::RangePolicy<CpExec>(space, 0, numContacts), KOKKOS_LAMBDA(int idx) {
        ...
        const F3 vAc = add3(ldF3(velPred, realA), cross3v(ldF3(angVelPred, realA), rA));
        F3 vBc{0, 0, 0};
        if (idB >= 0) vBc = add3(ldF3(velPred, realB), cross3v(ldF3(angVelPred, realB), rB));
