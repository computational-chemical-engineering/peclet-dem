/// @file
/// @brief dem — portable (CUDA-free) I/O helpers for the Kokkos dem module: a LAMMPS dump writer
/// and a scalar-SDF VTI writer. Operate on flat std::vector<float> host arrays (no CUDA
/// vector_types), so they compile on every Kokkos backend. Faithful to the CUDA io/Exporter.cpp
/// LAMMPS format and Simulation:: export_sdf VTI convention.
#ifndef DEM_IO_HPP
#define DEM_IO_HPP
...
// pos/vel are flat [n*3]; quat is flat [n*4] in (x,y,z,w) order; radii is [n]. Bounds: explicit box
// if provided, else computed from the particle AABBs + a 5% margin (matches
// io/Exporter.cpp::compute_bounds).
inline void writeLammpsDump(const std::string& filename, int step, const std::vector<float>& pos,
                            ...
                            const std::vector<float>& radii, const float* boxMin,
                            const float* boxMax, bool pbcEnabled) {
  ...
  if (boxMin && boxMax) {
    ...
    for (int d = 0; d < 3; ++d) {
      bmin[d] = std::numeric_limits<float>::max();
      bmax[d] = -std::numeric_limits<float>::max();
    }
    ...
    for (int d = 0; d < 3; ++d) {
      const float m = 0.05f * (bmax[d] - bmin[d]);
      bmin[d] -= m;
      bmax[d] += m;
    }
  }
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    file << (i + 1) << " 1 " << pos[3 * i] << " " << pos[3 * i + 1] << " " << pos[3 * i + 2] << " "
         << r << " " << vx << " " << vy << " " << vz << " " << qw << " " << qx << " " << qy << " "
         << qz << "\n";
  }
}

// Scalar-SDF ImageData VTI (ParaView), x-fastest grid [rx*ry*rz], origin = domain min, spacing =
// extent/res.
inline void writeSdfVti(const std::string& filename, const std::vector<float>& grid, int rx, int ry,
                        int rz, const float* minB, const float* maxB) {
  ...
  out << "  <ImageData WholeExtent=\"0 " << (rx - 1) << " 0 " << (ry - 1) << " 0 " << (rz - 1)
      << "\" Origin=\"" << minB[0] << " " << minB[1] << " " << minB[2] << "\" Spacing=\"" << sx
      << " " << sy << " " << sz << "\">\n";
