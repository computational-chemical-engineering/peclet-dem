...
inline std::vector<float> generateSdfKokkos(int rx, int ry, int rz, F3 dmin, F3 dmax, int numReal,
  ...
  Kokkos::parallel_for("dem::sdf::splat", Kokkos::RangePolicy<CpExec>(space, 0, numReal), KOKKOS_LAMBDA(int i) {
    ...
    float rbound = 1.0f;
    if (shp.type == HOLLOW_CYLINDER) { const float r = shp.params.x, h = shp.params.y; rbound = Kokkos::sqrt(r * r + (h * 0.5f) * (h * 0.5f)); }
    else if (shp.type == SPHERE) rbound = shp.params.x;
    rbound *= sc; rbound *= 1.2f;

    int minx = (int)Kokkos::floor((pw.x - rbound - origin.x) / vox.x), maxx = (int)Kokkos::ceil((pw.x + rbound - origin.x) / vox.x);
    int miny = (int)Kokkos::floor((pw.y - rbound - origin.y) / vox.y), maxy = (int)Kokkos::ceil((pw.y + rbound - origin.y) / vox.y);
    int minz = (int)Kokkos::floor((pw.z - rbound - origin.z) / vox.z), maxz = (int)Kokkos::ceil((pw.z + rbound - origin.z) / vox.z);
    if (!px) { minx = minx < 0 ? 0 : minx; maxx = maxx > rx - 1 ? rx - 1 : maxx; }
    if (!py) { miny = miny < 0 ? 0 : miny; maxy = maxy > ry - 1 ? ry - 1 : maxy; }
    if (!pz) { minz = minz < 0 ? 0 : minz; maxz = maxz > rz - 1 ? rz - 1 : maxz; }

    for (int z = minz; z <= maxz; ++z) for (int y = miny; y <= maxy; ++y) for (int x = minx; x <= maxx; ++x) {
      const F3 vp{origin.x + (x + 0.5f) * vox.x, origin.y + (y + 0.5f) * vox.y, origin.z + (z + 0.5f) * vox.z};
      const F3 prel = sub3(vp, pw);
      const F3 plocal = scale3(invRotateVector(q, prel), 1.0f / sc);
      float distc = 100.0f;
      if (shp.type == HOLLOW_CYLINDER) distc = sdfHollowCylinder(plocal, shp.params);
      else if (shp.type == SPHERE) distc = sdfSphere(plocal, shp.params);
      const float dist = distc * sc;
      const int wx = (x % rx + rx) % rx, wy = (y % ry + ry) % ry, wz = (z % rz + rz) % rz;
      if (!px && (x < 0 || x >= rx)) continue;
      if (!py && (y < 0 || y >= ry)) continue;
      if (!pz && (z < 0 || z >= rz)) continue;
      const long vi = (long)wz * rx * ry + (long)wy * rx + wx;
      Kokkos::atomic_min(&grid(vi), dist);
      if (dist < rbound) state(vi) = 1;
    }
  });
  space.fence();
  ...
  for (int it = 0; it < iters; ++it) {
    ...
    Kokkos::parallel_for("dem::sdf::eikonal", Kokkos::RangePolicy<CpExec>(space, 0, total), KOKKOS_LAMBDA(long idx) {
      ...
      if (x > 0) xm = din(idx - 1); else if (px) xm = din(idx - 1 + rx);
      if (x < rx - 1) xp = din(idx + 1); else if (px) xp = din(idx + 1 - rx);
      if (y > 0) ym = din(idx - sy); else if (py) ym = din((long)z * sz + (long)(ry - 1) * sy + x);
      if (y < ry - 1) yp = din(idx + sy); else if (py) yp = din((long)z * sz + x);
      if (z > 0) zm = din(idx - sz); else if (pz) zm = din((long)(rz - 1) * sz + (long)y * sy + x);
      if (z < rz - 1) zp = din(idx + sz); else if (pz) zp = din((long)y * sy + x);
      ...
      if (u1 <= b) u = u1;
      else { const float d2 = 2.0f * h * h - (a - b) * (a - b); float u2 = (d2 < 0.0f) ? 100.0f : (a + b + Kokkos::sqrt(d2)) * 0.5f;
        if (u2 <= c) u = u2;
        else { const float s = a + b + c, qd = s * s - 3.0f * (a * a + b * b + c * c - h * h);
  ...
  auto hv = Kokkos::create_mirror_view(in); Kokkos::deep_copy(hv, in);
  for (long i = 0; i < total; ++i) h_grid[i] = hv(i);
  return h_grid;
}
