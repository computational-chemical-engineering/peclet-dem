...
///
/// Exposes peclet::dem::Simulation (the portable Kokkos+ArborX pipeline) with the essential sphere-packing API.
/// Kokkos is initialized at import and left initialized for the interpreter's lifetime. Particle
/// arrays cross the boundary through the shared peclet::core::python bridge (transport-core): inputs read as
...
static nb::ndarray<nb::numpy, float> rows(std::vector<float>&& v, int cols) {
  ...
  return peclet::core::python::vector_to_ndarray(std::move(v), {n, (std::size_t)cols}, {(std::int64_t)cols, 1});
}
...
NB_MODULE(_dem, m) {
  ...
  nb::class_<Simulation>(m, "Simulation")
      ...
      .def("set_sphere_shape", &Simulation::setSphereShape, nb::arg("radius"), "Use a uniform sphere of the given radius for all particles.")
      ...
      .def("set_domain",
           [](Simulation& s, std::tuple<float, float, float> mn, std::tuple<float, float, float> mx) {
      ...
      .def("enable_periodicity", &Simulation::enablePeriodicity, nb::arg("x"), nb::arg("y"), nb::arg("z"), "Enable periodic boundaries per axis (x, y, z).")
      .def("get_domain_min", &Simulation::getDomainMin, "Return the domain minimum corner (x, y, z).")
      .def("get_domain_max", &Simulation::getDomainMax, "Return the domain maximum corner (x, y, z).")
      .def("set_gravity", &Simulation::setGravity, "Set the gravitational acceleration vector (gx, gy, gz).")
      .def("set_thermostat", &Simulation::setThermostat,
      ...
      .def("set_solver_iterations", &Simulation::setSolverIterations, nb::arg("pos"), nb::arg("vel"), "Set the XPBD position- and velocity-solve iteration counts.")
      .def("set_global_scale", &Simulation::setGlobalScale, "Set a global length scale applied to all particles.")
      .def("set_dt", &Simulation::setDt, "Set the time step dt.")
      .def("set_material_params", &Simulation::setMaterialParams,
      ...
      .def("set_velocities", [](Simulation& s, nb::ndarray<float, nb::c_contig> a) { s.setVelocities(to_vec(a)); }, "Set particle velocities from an (N,3) array.")
      .def("set_quaternions", [](Simulation& s, nb::ndarray<float, nb::c_contig> a) { s.setQuaternions(to_vec(a)); }, "Set particle orientation quaternions from an (N,4) array.")
      .def("set_angular_velocities", [](Simulation& s, nb::ndarray<float, nb::c_contig> a) { s.setAngularVelocities(to_vec(a)); }, "Set particle angular velocities from an (N,3) array.")
      .def("set_inv_inertia", [](Simulation& s, nb::ndarray<float, nb::c_contig> a) { s.setInvInertia(to_vec(a)); }, "Set per-particle inverse inertia from an (N,3) array.")
      .def("set_inv_mass", [](Simulation& s, nb::ndarray<float, nb::c_contig> a) { s.setInvMass(to_vec(a)); }, "Set per-particle inverse mass (0 => fixed/immovable).")
      .def("get_angular_velocities", [](const Simulation& s) { return rows(s.getAngularVelocities(), 3); })
      .def("get_inv_inertia", [](const Simulation& s) { return rows(s.getInvInertia(), 3); })
      .def("set_scales_uniform", &Simulation::setScalesUniform, "Set a single uniform scale for all particles.")
      .def("set_scales", [](Simulation& s, nb::ndarray<float, nb::c_contig> a) { s.setScales(to_vec(a)); }, "Set per-particle scales from an array.")
      .def("set_growth_params", &Simulation::setGrowthParams, nb::arg("rate"), nb::arg("new_factor") = -1.0f, "Set the particle growth rate and target size factor.")
      .def("get_growth_factor", &Simulation::growthFactor, "Return the current particle growth factor.")
      ...
      .def("get_positions", [](const Simulation& s) { return rows(s.getPositions(), 3); },
           "Return particle positions as an (N,3) numpy array.")
      .def("get_velocities", [](const Simulation& s) { return rows(s.getVelocities(), 3); },
      ...
      .def("get_quaternions", [](const Simulation& s) { return rows(s.getQuaternions(), 4); },
           "Return particle orientation quaternions as an (N,4) numpy array.")
      .def("get_scales", [](const Simulation& s) { return flat(s.getScales()); },
           "Return per-particle scales as a numpy array.")
      .def("step", &Simulation::step, nb::arg("dt") = 0.0f, "Advance the simulation one step (dt=0 uses the configured time step).")
      .def("get_sdf_grid",
           [](Simulation& s, std::tuple<int, int, int> res) {
             auto [rx, ry, rz] = res;
             // C-order (rx,ry,rz) float array, matching the prior py::array_t<float>({rx,ry,rz}, ...).
             return peclet::core::python::vector_to_ndarray(s.getSdfGrid(rx, ry, rz),
      ...
      .def("write_vtp", &Simulation::writeVtp, nb::arg("filename"), "Write particle state to a VTP file (ParaView/Ovito).")
      ...
      .def("export_lammps", &Simulation::exportLammps, nb::arg("filename"), nb::arg("step"), "Export particle state to a LAMMPS dump file.")
      .def("export_sdf",
      ...
      .def("enable_mpi_step", &Simulation::enableMpiStep,
      ...
      .def("step_mpi", &Simulation::stepMpi, nb::arg("nsteps") = 1, "Advance the distributed (MPI) simulation by nsteps with halo exchange.")
  ...
  m.def("export_lammps",
        [](const std::string& filename, int step, nb::ndarray<float, nb::c_contig> pos,
          ...
          peclet::dem::writeLammpsDump(filename, step, to_vec(pos), to_vec(vel), to_vec(quats), to_vec(radii),
                               pmn, pmx, pbc_enabled);
        },
