...
core's peclet.halo (block decomposition + particle migration/ghosts).
...
Run (system python3 has mpi4py + numpy; build both modules first):
    cmake -S . -B build -DPECLET_DEM_MPI=ON && cmake --build build -j
    cmake -S ../core/python -B ../core/python/build && \
        cmake --build ../core/python/build -j
    PYTHONPATH=<dem-build>:<core-python-build> \
        mpirun -np 4 python3 examples/driver_distributed.py
...
radius = 0.5
rcut = 2.0 * radius
mig = halo.ParticleMigrator(origin=dmin, extent=L, cells=[32, 32, 32], periodic=[True, True, True])
...
for step in range(5):
    ...
    # quaternion + angular velocity through migration (see tests/python/mpi/test_verify_distributed.py) -- dropping
